...
-   Base your food suggestions on common Indian cuisine.`;
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      let errorMessage = "I encountered an error. Could you please rephrase your message or try again?";
      if (error instanceof Error) {
        if (error.message.includes('API key')) {
          errorMessage = "There's a connection issue. The API key might be invalid or missing.";
        } else if (error.message.toLowerCase().includes('blocked')) {
          errorMessage = "My response was blocked due to safety settings. Please try a different prompt.";
        }
      }
      setMessages(prev => {
        const newMessages = [...prev];
